defmodule FeedbackApiWeb.Api.V2.Staff.InstructorControllerTest do
  ...
  setup do
    instructor_1 =
      %User{name: "Mike Dao", api_key: "redlobster", role: :Instructor} |> Repo.insert!()

    instructor_2 =
      %User{name: "Josh Mejia", api_key: "getofftheshed", role: :Instructor} |> Repo.insert!()

    inactive_instructor =
      %User{name: "Hashtag Steve", api_key: "inactive", status: :Inactive, role: :Instructor}
      |> Repo.insert!()
  ...
  test "Returns a list of all other active staff members in the system", %{
    conn: conn,
    users: [mike, josh, _inactive, _student]
  } do
  ...
  test "Returns a 401 and invalid API key message if student credentials provided", %{
    conn: conn,
    users: [_mike, _josh, _inactive, student]
  } do
  ...
  test "Returns a 401 and invalid key message if account is inactive", %{
    conn: conn,
    users: [_mike, _josh, inactive, _student]
  } do
